...
public final class NumberGenerateUtils {
     ...
     * @param fixBatch 批次
     * @param fixBatchPosi 批次占用数字字符串位置
     * @param groupNum 批次中每位数字的变化组数
     * @param length 数据总长度,最大支持50
     */
    public static NumberGenerateUtils getInstance(int fixBatch, int[] fixBatchPosi, BATCHEACHBIT_GROUP_NUM groupNum,
        ...
        synchronized (lock) {
            ...
            if (fixBatch == numberUtils.fixBatch) {
                ...
                numberUtils.switchBatch(fixBatch, fixBatchPosi, groupNum, length);
                return numberUtils;
            }
        }
    }
    ...
    public String next() {
        ...
        //获取批次字符串
        String fixBatchStr = StringUtils.leftPad(RadixUtils.convertNumber(this.fixBatch, groupNum), fixBatchPosi.length, "0");
     ...
     * 切换到下一个批次
     */
    private void switchBatch(int fixBatch, int[] fixBatchPosi, BATCHEACHBIT_GROUP_NUM groupNum, int length) {
        reset(fixBatch, fixBatchPosi, groupNum, length);
    }
     ...
     * 重置批次 
     */
    private void reset(int fixBatch, int[] fixBatchPosi, BATCHEACHBIT_GROUP_NUM groupNum, int length) {
        if (groupNum == null) {
            throw new IllegalArgumentException("批次中每位数字的变化组数不能为空");
        }
        
        this.fixBatchPosi = fixBatchPosi;
        this.groupNum = groupNum.value;
        this.length = length;
    ...
    public static void main(String[] args) {
        //NumberGenerateUtils.getInstance(-1, new int[] {1, 2, 11}, BATCHEACHBIT_GROUP_NUM.FIVE, 10);
        // NumberGenerateUtils.getInstance(0, new int[] {1, 2, 3}, BATCHEACHBIT_GROUP_NUM.FIVE, 10);
        // NumberGenerateUtils.getInstance(124, new int[] {1, 2, 3}, BATCHEACHBIT_GROUP_NUM.FIVE, 10);
        //NumberGenerateUtils.getInstance(125, new int[] {1, 2, 3}, BATCHEACHBIT_GROUP_NUM.FIVE, 3);
        
        // NumberGenerateUtils.getInstance(1, new int[] {1, 2, 3,4,5,6}, BATCHEACHBIT_GROUP_NUM.FIVE, 5);
        // NumberGenerateUtils.getInstance(1, new int[] {1, 2, 11}, BATCHEACHBIT_GROUP_NUM.FIVE, 5);
        //  NumberGenerateUtils.getInstance(1, new int[] {1, 2, 1}, BATCHEACHBIT_GROUP_NUM.FIVE, 5);
        //  NumberGenerateUtils.getInstance(125, new int[] {1, 2, 3}, BATCHEACHBIT_GROUP_NUM.FIVE, 10);
        // NumberGenerateUtils.getInstance(1, new int[] {1, 1, 3}, BATCHEACHBIT_GROUP_NUM.FIVE, 10);
        NumberGenerateUtils numberUtils =
            NumberGenerateUtils.getInstance(124, new int[] {1, 2, 3}, BATCHEACHBIT_GROUP_NUM.FIVE, 4);
        for(int i = 0; i < 10; ++i){
