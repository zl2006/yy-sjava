 ...
 */
public class HessianParser extends AbstraceParser {
    
    // 访问协议
    private static final String PROTOCAL = "http://";
    
    public HessianParser(ServiceScannerConfigurer scannerConfigurer) { 
        super(scannerConfigurer,Hessian.class);
    }
    
    /** {@inheritDoc} */
    public String buildServerBeanName(ScannedGenericBeanDefinition beanDefinition) {
        Map<String, Object> annotationAttributes =
            beanDefinition.getMetadata().getAnnotationAttributes(annoClass.getName());
        String uri = (String)annotationAttributes.get("uri") + "/" + scannerConfigurer.getVersion();
        return uri;
    }
    ...
    public String buildClientBeanName(ScannedGenericBeanDefinition beanDefinition) {
        ...
        Map<String, Object> annotationAttributes = metadata.getAnnotationAttributes(annoClass.getName());
    ...
    public void parseClientBeanDefine(ScannedGenericBeanDefinition srcBeanDefinition,
        ...
        targetBeanDefinition.getPropertyValues().add("serviceUrl",
            PROTOCAL + scannerConfigurer.getRemoteUrls().get(app) + uri + "/" + scannerConfigurer.getVersion()); //服务远程调用地址设置
